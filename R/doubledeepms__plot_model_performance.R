...
doubledeepms__plot_model_performance <- function(
  ...
  d <- ggplot2::ggplot(plot_dt,ggplot2::aes(observed_fitness, predicted_fitness)) +
    ...
    ggplot2::geom_abline(color = highlight_colour, linetype = 2, size = 1) +
  ...
  d <- ggplot2::ggplot(plot_dt,ggplot2::aes(observed_fitness, predicted_fitness)) +
    ...
    ggplot2::ylab("Predicted fitness") +
    ggplot2::geom_abline(color = highlight_colour, linetype = 2, size = 1) +
    ggplot2::geom_text(data = plot_dt[,.(label = paste("Pearson's r = ", round(cor(observed_fitness, predicted_fitness, use = "pairwise.complete"), 2), sep="")),.(dataset)], ggplot2::aes(label=label, x=-Inf, y=Inf, hjust = 0, vjust = 1)) +
