...
doubledeepms__plot_additive_trait_folding <- function(
  ...
  pred_fitness_dt <- data.table(
    additive_trait_folding = rep(folding_energy_grid, 2),
    additive_trait_binding = rep(binding_energy_grid, 2),
  ...
  #Binhex no facet - xlim [-5, 10]
  plot_xlim <- c(-5, 10)
  d <- ggplot2::ggplot(plot_dt[mut_order>0 & additive_trait_folding>plot_xlim[1] & additive_trait_folding<plot_xlim[2]],ggplot2::aes(additive_trait_folding, observed_fitness)) +
    ggplot2::stat_binhex(bins = 100, size = 0, color = "lightgrey") +
    ggplot2::scale_fill_gradientn(colours = c("white", "black"), trans = "log10") +
    ggplot2::xlab(expression(Delta*"G Folding (ddPCA)")) +
    ggplot2::ylab("Fitness (Abundance)") +
    ...
    ggplot2::geom_vline(xintercept = 0, linetype = 2) +
    ggplot2::geom_vline(xintercept = plot_dt[mut_order==0,additive_trait_folding][1], color = "grey", linetype = 2) +
    ggplot2::geom_line(data = pred_fitness_dt[mut_order==1 & additive_trait_folding>plot_xlim[1] & additive_trait_folding<plot_xlim[2]], color = colour_scheme[["shade 0"]][[1]]) +
