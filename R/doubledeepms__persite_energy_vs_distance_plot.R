...
doubledeepms__persite_energy_vs_distance_plot <- function(
  ...
  # plot_dt[, plot_shape := class=="Allosteric"]
  # plot_shapes <- c(19, 1)
  # names(plot_shapes) <- c("FALSE", "TRUE")
  ...
  if(trait_name=="binding"){
    d <- d + ggplot2::geom_smooth(color = "black", method = "glm", formula = y ~ exp(-x)) +
  ...
  ggplot2::ggsave(outpath, d, width = 4, height = 3, useDingbats=FALSE)
